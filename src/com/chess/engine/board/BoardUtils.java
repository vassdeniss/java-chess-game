...
public enum BoardUtils {
    ...
    private static List<Boolean> initColumn(int columnNumber) {
        ...
        for(int i = 0; i < column.length; i++) { column[i] = false; }
    ...
    private Map<String, Integer> initializePositionToCoordinateMap() {
        ...
        for (int i = START_TILE_INDEX; i < TILES; i++) { positionToCoordinate.put(ALGEBRAIC_NOTATION.get(i), i); }
    ...
    public static boolean isValidTileCoordinate(final int coordinate) {
        return coordinate >= START_TILE_INDEX && coordinate < TILES;
    }
    public int getCoordinateAtPosition(final String position) { return POSITION_TO_COORDINATE.get(position); } // Methods for returning algebraic notation
    public String getPositionAtCoordinate(final int coordinate) { return ALGEBRAIC_NOTATION.get(coordinate); }
    public static boolean isThreatenedBoardImmediate(final Board board) { return board.whitePlayer().isInCheck() || board.blackPlayer().isInCheck(); }
    public static boolean kingThreat(final Move move) {
        final Board board = move.getBoard();
        final MakeTransition transition = board.currentPlayer().makeMove(move);
