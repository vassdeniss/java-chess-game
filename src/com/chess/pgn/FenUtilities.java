...
public class FenUtilities {
    private FenUtilities() { throw new RuntimeException("Not Instantiable!"); }

    public static Board createGameFromFEN(final String fenString) { return parseFEN(fenString); }

    public static void createFENFromGame(final File FENFile, final Board board) throws IOException {
        final StringBuilder builder = new StringBuilder();
        builder.append(calculateBoardText(board)).append(" ");
        builder.append(calculateCurrentPlayerText(board)).append(" ");
        builder.append(calculateCastleText(board)).append(" ");
        builder.append(calculateEnPassantSquare(board)).append(" ");
        builder.append("0 1");
    ...
    public static String testFENCreate(final Board board) {
    ...
    private static Board parseFEN(final String fenString) {
        ...
        builder.setMoveDecider(moveMaker(fenPartitions[1]));
        return builder.build();
    }

    private static Alliance moveMaker(final String moveMakerString) {
        if (moveMakerString.equals("w")) { return Alliance.WHITE; }
        else if (moveMakerString.equals("b")) { return Alliance.BLACK; }
        throw new RuntimeException("Invalid FEN String " + moveMakerString);
    }

    private static boolean whiteKingSideCastle(final String fenCastleString) { return fenCastleString.contains("K"); }
    private static boolean whiteQueenSideCastle(final String fenCastleString) { return fenCastleString.contains("Q"); }
    private static boolean blackKingSideCastle(final String fenCastleString) { return fenCastleString.contains("k"); }
    private static boolean blackQueenSideCastle(final String fenCastleString) { return fenCastleString.contains("q"); }
    ...
    private static String calculateCurrentPlayerText(final Board board) {
        System.out.println(board.currentPlayer().toString().toLowerCase());
